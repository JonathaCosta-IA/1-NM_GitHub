
\section{Conversão - norma IEEE 754/2008}
\label{sec:conversão}

\begin{enumerate}[label=\thesection.\arabic*]
	% *****************************************************************
\item Represente os números a seguir nos formatos abaixo solicitados: 
{\bf 81, 66.25, -0.625, 0.533203125}.
    \begin{enumerate}
        \item Binário convencional;
        \item Binário em ponto flutuante;
        \item Cadeia de 32 bits em precisão simples conforme a norma IEEE--754;
        \item Cadeia de 64 bits em precisão dupla conforme a norma IEEE--754.
    \end{enumerate}
    
\item Faça um {\it script} que receba um número decimal {\bf positivo} e converta-o para:
    \begin{enumerate}
        \item Binário convencional;
        \item Binário em ponto flutuante;
        \item Cadeia de 32 bits em precisão simples conforme a norma IEEE--754;
        \item Cadeia de 64 bits em precisão dupla conforme a norma IEEE--754;
    \end{enumerate}
    
\item Faça um {\it script} que receba um número decimal {\bf negativo} e converta-o para:
	\begin{enumerate}
        \item Binário convencional;
        \item Binário em ponto flutuante;
        \item Cadeia de 32 bits em precisão simples conforme a norma IEEE--754;
        \item Cadeia de 64 bits em precisão dupla conforme a norma IEEE--754;
    \end{enumerate}
    
\item Faça um {\it script} que receba um número decimal com {\bf mantissa} (parte não inteira) e converta-o para:
\begin{enumerate}
        \item Binário convencional;
        \item Binário em ponto flutuante;
        \item Cadeia de 32 bits em precisão simples conforme a norma IEEE--754;
        \item Cadeia de 64 bits em precisão dupla conforme a norma IEEE--754;
\end{enumerate}

\item Utilizando POO, ajuste o {\it script} da questão anterior para receber o número decimal em quaisquer modos supracitados e convertê-lo para as saídas 
    anteriormente exploradas.

\end{enumerate}
%=======================================================================
\clearpage
\section{Diferenciação Numérica} \label{sec:diferenciação}

\begin{enumerate}[label=\thesection.\arabic*]
\item  Considere a função $f(x) = x^3$. Calcule numericamente a derivada primeira no ponto $x = 3$
 aplicando as fórmulas de diferenças finitas progressiva, regressiva e central, utilizando:
\begin{itemize}
    \item os pontos x = 2, x = 3 e x = 4.
    \item os pontos x = 2,75, x = 3 e x = 3,25.
\end{itemize} 
Compare os resultados com a derivada exata (analítica) e expresse os erros relativo e absoluto.


\item Considere a função $f(x) = \ln(x)$. 
Utilize a fórmula de diferença progressiva de primeira ordem para aproximar $f'(1)$ com $h = 0{,}1$. 
Compare com o valor exato da derivada e calcule o erro absoluto.

\item Dada a função $f(x) = e^{-x}$, utilize a fórmula de diferença centrada de segunda ordem para estimar $f'(0)$ com $h = 0{,}05$. 
Analise a ordem do erro de truncamento.

\item A tabela abaixo fornece valores experimentais de uma função:

\[
\begin{array}{c|c}
x & f(x) \\
\hline
1{,}0 & 2{,}7183 \\
1{,}1 & 3{,}0042 \\
1{,}2 & 3{,}3201 \\
\end{array}
\]

Utilize a fórmula de diferença regressiva para estimar $f'(1{,}2)$.

\item Considere a função $f(x) = x^3 - 2x + 1$. 
Utilize:
\begin{itemize}
\item diferença progressiva,
\item diferença regressiva,
\item diferença centrada,
\end{itemize}
para estimar $f'(2)$ com $h = 0{,}1$. 
Compare os resultados e identifique qual método apresenta menor erro.

\item A função $f(x) = \sin(x)$ é avaliada numericamente. 
Utilize a fórmula de três pontos centrada para aproximar $f''(\pi/4)$ com $h = 0{,}01$. 
Compare com o valor analítico da segunda derivada.

\item Em um experimento de vibração, um bloco de massa $m$ é preso
a uma mola com dureza $k$ e a um amortecedor com coeficiente de
amortecimento $c$, conforme mostrado na Figura~\ref{fig:sistema_massa_mola}.

\noindent
\begin{minipage}{0.5\textwidth}
    O sistema massa-mola-amortecedor é descrito por uma equação diferencial
    de segunda ordem. Esse modelo é a base clássica da dinâmica estrutural,
    permitindo analisar regimes subamortecido, criticamente amortecido e
    superamortecido, além da resposta transitória e em regime permanente.
\end{minipage}
\hfill
\begin{minipage}{0.4\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figuras/SistemaMassaMola}
    \captionof{figure}{Sistema massa-mola}
    \label{fig:sistema_massa_mola}
\end{minipage}


Para que o experimento tenha início, o bloco é retirado da posição de equilíbrio e solto. 
A posição do bloco em função do tempo é gravada em uma freqüência
de $5 Hz$ (5 vezes por segundo). Os dados no intervalo $4 \le t \le 8$ s são mostrados a seguir.
\[
\begin{aligned}
t = \{&4.0,\, 4.2,\, 4.4,\, 4.6,\, 4.8,\, 5.0,\, 5.2,\, 5.4,\, 5.6,\, 5.8,\,6.0,\, 6.2,\, 6.4,\, 6.6,\, 6.8,\, 7.0,\, 7.2,\, 7.4
\\ & 7.6,\, 7.8,\, 8.0\}\\
x = \{&-5.87,\, -4.23,\, -2.55,\, -0.89,\, 0.67,\, 2.09,\, 3.31,\, 4.31,\, 5.06,\,5.50,\, 5.77,\, 5.52,\, 5.08\\
& 4.46,\, 3.72,\, 2.88,\, 2.00,\, 1.10,\, 0.23,\, -0.59\}
\end{aligned}
\]
Com base nos dados deste experimento calcule:
    \begin{enumerate}
        \item A velocidade do bloco é a derivada da posição em
    relação ao tempo. Utilize a fórmula de diferença finita central para calcular a velocidade nos tempos $t = 5$s e $t = 6$ s.
        \item Escreva um {\it script} que calcule a derivada de uma função descrita por um conjunto de pontos. 
        Denomine a função de $dx=derivada(x,y)$,em que $x$ e $y$ são vetores com as coordenadas dos pontos, e $dx$ é um vetor com a derivada $dy/dx$ em
    cada ponto. A função deve calcular a derivada no primeiro e no último ponto usando as fórmulas de diferenças finitas progressiva e regressiva, respectivamente, 
    e usando a fórmula de diferença finita central nos demais pontos. 
        \item Utilize os pontos fornecidos para calcular a velocidade do bloco em $4 \le t \le 8$s. 
        \item Calcule a aceleração do bloco a partir da diferenciação da velocidade. 
        \item Trace um gráfico contendo deslocamento, velocidade e aceleração versus tempo para $4 \le t \le 8$s.
    \end{enumerate}


\item Em um ensaio de tração uniaxial de um aço, foram obtidos os seguintes dados experimentais de tensão ($\sigma$) 
em função da deformação ($\varepsilon$):
\[
\begin{array}{c|c}
\varepsilon & \sigma \; (\text{MPa}) \\
\hline
0.0000 &   0.0 \\
0.0002 &  42.0 \\
0.0004 &  85.0 \\
0.0006 & 128.0 \\
0.0008 & 170.0 \\
0.0010 & 210.0 \\
0.0012 & 250.0 \\
0.0014 & 295.0 \\
0.0016 & 340.0 \\
0.0018 & 380.0 \\
0.0020 & 420.0 \\
0.0022 & 455.0 \\
0.0024 & 485.0 \\
0.0026 & 510.0 \\
0.0028 & 535.0 \\
0.0030 & 555.0 \\
\end{array}
\]

\begin{enumerate}
\item Utilize a fórmula de diferença centrada para estimar numericamente o módulo de elasticidade tangente
 $ E_i = \dfrac{d\sigma}{d\varepsilon}$ nos pontos internos da tabela.

\item Utilize fórmulas de diferença progressiva e regressiva para estimar $E$ nos pontos extremos.

\item Com base nos valores obtidos, identifique o intervalo de deformação no qual o material apresenta comportamento aproximadamente linear.

\item Estime o módulo de elasticidade médio $E$ do material na região elástica e compare com o valor típico para o aço ($E \approx 200$ GPa).

\item Discuta a influência da discretização dos dados na precisão da estimativa de $E$, 
considerando erros de truncamento e possíveis ruídos experimentais.

\end{enumerate}


\item Utilizando POO, ajuste o \textit{script} da questão anterior para receber os vetores de $\sigma, \epsilon$ e
retornar a análise gráfica conforme realizado na questão acima.


\item Um sistema de aquisição de dados registrou medições discretas de tensão $V(t)$ e corrente $I(t)$ de 
uma carga elétrica ao longo do tempo. Os dados obtidos são apresentados na tabela a seguir:

\[
\begin{array}{c|c|c}
t \; (\text{s}) & U \; (\text{V}) & I \; (\text{A}) \\
\hline
0.0 & 220.000 & 0.52 \\
0.5 & 220.495 & 0.61 \\
1.0 & 220.958 & 0.68 \\
1.5 & 221.363 & 0.74 \\
2.0 & 221.683 & 0.79 \\
2.5 & 221.898 & 0.76 \\
3.0 & 221.995 & 0.70 \\
3.5 & 221.968 & 0.99 \\
4.0 & 221.818 & 1.49 \\
4.5 & 221.556 & 0.36 \\
5.0 & 221.197 & 0.28 \\
5.5 & 220.763 & 0.19 \\
6.0 & 220.282 & 1.14 \\
6.5 & 219.784 & 1.15 \\
7.0 & 219.298 & 0.18 \\
7.5 & 218.856 & 1.27 \\
8.0 & 218.486 & 0.38 \\
8.5 & 218.212 & 0.47 \\
9.0 & 218.045 & 0.59 \\
9.5 & 217.990 & 0.71 \\
10.0 & 218.082 & 0.78 \\
\end{array}
\]

Com base nesses dados, responda:

\begin{enumerate}

\item Calcule a potência instantânea $P(t_i)$ em cada ponto da tabela utilizando:
\[
P(t_i) = V(t_i)\,I(t_i).
\]

\item Utilize as seguintes aproximações de diferenças finitas para estimar numericamente a derivada da potência:

\begin{itemize}
\item Diferença progressiva:
\[
\left.\frac{dP}{dt}\right|_{t_i} \approx \frac{P_{i+1} - P_i}{t_{i+1} - t_i}
\]

\item Diferença regressiva:
\[
\left.\frac{dP}{dt}\right|_{t_i} \approx \frac{P_i - P_{i-1}}{t_i - t_{i-1}}
\]

\item Diferença centrada:
\[
\left.\frac{dP}{dt}\right|_{t_i} \approx \frac{P_{i+1} - P_{i-1}}{t_{i+1} - t_{i-1}}
\]
\end{itemize}

\item Compare os três métodos de diferenciação numérica e discuta suas diferenças em termos de precisão e 
comportamento nas extremidades do intervalo.

\item Construa os seguintes gráficos em um arranjo $2\times 2$:
\begin{itemize}
\item $V(t)$ em função do tempo;
\item $I(t)$ em função do tempo;
\item $P(t)$ em função do tempo;
\item comparação entre $\dfrac{dP}{dt}$ obtido pelos três métodos.
\end{itemize}

\item Determine os instantes de tempo em que ocorrem:
\begin{itemize}
\item o maior aumento de potência;
\item a maior queda de potência.
\end{itemize}

\item Com base nos resultados obtidos, interprete o comportamento dinâmico da carga, identificando possíveis transientes e variações abruptas de consumo.

\item Discuta a influência do passo de amostragem $\Delta t$ na precisão das derivadas numéricas e na representação gráfica dos dados.

\end{enumerate}

\item Utilizando POO, ajuste o \textit{script} da questão anterior para receber os vetores de $I,V,t$ e retornar a análise gráfica conforme realizado
na questão acima.
\end{enumerate}

\clearpage
\section{Integração Numérica}\label{sec:integração}

\begin{enumerate}[label=\thesection.\arabic*]
\item Implemente um \textit{script} que realize a aproximação da integral definida utilizando o método do trapézio simples.

Aplique a função para calcular:
\[
\int_{0}^{2} (x^2 + 1)\,dx
\]

Determine também o erro absoluto em relação ao valor analítico da integral.

\item Implemente o método do trapézio composto e utilize-o para aproximar a integral:
\[
\int_{0}^{1} e^x \, dx
\]

Considere os valores $n = 4, 8, 16, 32, 48$ subintervalos. Apresente os resultados em forma de tabela e analise a convergência do método.
%
\item Implemente o método de Simpson 1/3, garantindo que o número de subintervalos $n$ seja par. Utilize o método para calcular:
\[
\int_{0}^{2} \ln(x+1)\,dx
\]

Compare os resultados com aqueles obtidos pelo método do trapézio composto utilizando o mesmo número de subintervalos.

\item Para a integral:
\[
\int_{0}^{\pi} \sin(x)\,dx
\]

Implemente e compare os seguintes métodos:
\begin{itemize}
    \item Método do ponto médio
    \item Método do trapézio
    \item Método de Simpson 1/3
\end{itemize}

Utilize $n = 4, 10, 50$ subintervalos. Construa um gráfico do erro absoluto em função de $n$ e discuta os resultados.

\item Considere a função:
\[
f(x) = \frac{1}{1 + x^2}
\]
no intervalo $[0,1]$.

\begin{enumerate}
    \item Calcule a integral utilizando:
    \begin{itemize}
        \item Método do trapézio composto
        \item Método de Simpson 1/3
    \end{itemize}
    \item Utilize $n = 2^k$, com $k = 1, 2, \dots, 6$
    \item Construa um gráfico em escala log-log do erro em função do passo $h$
    \item Estime numericamente a ordem de convergência de cada método
\end{enumerate}

\end{enumerate}

\clearpage
\section{Ajuste de curvas} \label{sec:ajusteCurvas}

\subsection*{Regressão linear}
\begin{enumerate}[label=\thesection.\arabic*]

\item Sejam os dados:
\[
\begin{array}{c|ccccccc}
x & 2 & 5 & 6 & 8 & 9 & 13 & 15 \\
\hline
y & 7 & 8 & 10 & 11 & 12 & 14 & 15
\end{array}
\]
\begin{itemize}
    \item Determine a reta $y = a + bx$ por mínimos quadrados 
    \item Produza um $novoY$ a partir da reta gerada.
    \item Calcule o erro quadrático total.
    \item Trace o gráfico com: $ x \times y , x \times novoY$ 
\end{itemize}


\item Ajuste, pelo método dos mínimos quadrados, um modelo linear do tipo $F = a + bx$ aos dados experimentais abaixo. 

% É esperado que o aluno reconheça que b = dF /dx ou seja, é a taxa de variação da força em relação ao deslocamento
% Comparado com o modelo clássico: F=kx
% Logo: b≈k => quanto maior (b) maior da rigidez do sistema; quanto menor mais flexível é o sistema.

\[
    \begin{array}{c|cccc}
        x & 0.1 & 0.2 & 0.3 & 0.4 \\
        F & 1.2 & 2.3 & 3.1 & 3.9
    \end{array}
    \]
    
\begin{itemize}
    \item Produza um $novoY$ a partir da reta gerada.
    \item Calcule o erro quadrático total.
    \item Trace o gráfico com: $ x \times y , x \times novoY$ 
    \item Interprete o coeficiente angular $b$ como a rigidez do sistema, à luz da relação força--deslocamento.
\end{itemize}


\item Ajuste, pelo método dos mínimos quadrados, um modelo linear aos dados apresentados e determine o coeficiente de determinação $R^2$. 
Analize a qualidade do ajuste obtido.
\[
\begin{array}{c|ccccc}
x & 0 & 1 & 2 & 3 & 4 \\
y & 1.1 & 2.0 & 2.9 & 4.2 & 4.8
\end{array}
\]
\begin{itemize}
    \item Produza um $novoY$ a partir da reta gerada.
    \item Calcule o erro quadrático total.
    \item Encontre o valor de y para um $ \delta\ | \ \delta\ \subset  \ x$.
    \item Trace o gráfico com: $ x \times y , x \times novoY$. 
\end{itemize}

\subsection*{Regressão Polinomial}

\item Sejam os vetores:
\[
x = [-2, -1, 0, 1, 2], \quad y = [4.1, 1.0, 0.2, 1.1, 3.9]
\]

\begin{itemize}
    \item Ajuste um polinômio $p(x)$ de grau 2 aos dados.
    \item Calcule o erro absoluto associado ao ajuste.
    \item Compare graficamente $y$ e $p(x)$ em função de $x$.
\end{itemize}

\item Sejam os vetores:
\[
x = [-1, 0, 1, 2, 3], \quad y = [1, 0, 1, 8, 27]
\]

\begin{itemize}
    \item Ajuste polinômios $p(x)$ de grau 2, 3 e 4.
    \item Calcule o erro absoluto para cada modelo.
    \item Determine o coeficiente de determinação $R^2$ para cada ajuste.
    \item Identifique o modelo mais adequado.
    \item Compare graficamente os ajustes obtidos.
\end{itemize}

\subsection*{Interpolação de Lagrange}

\item Sejam os vetores:

\[
x=
\begin{bmatrix}
8\\2\\11\\5\\1\\9\\4\\12\\7\\3\\10\\6
\end{bmatrix}
\qquad
y=
\begin{bmatrix}
64\\4\\121\\25\\1\\81\\16\\144\\49\\9\\100\\36
\end{bmatrix}
\]

\begin{itemize}
    \item Construa o polinômio interpolador de Lagrange.
    \item Estime $f(2.5)$.
    \item Analise o erro da interpolação, comparando com o valor exato da função $f(x) = x^2$.
    \item Apresente o resultado graficamente.
\end{itemize}

\item Sejam os vetores:
\[
x = [0, 1, 2], \quad y = [1, e, e^2]
\]

\begin{itemize}
    \item Construa o polinômio interpolador de Lagrange.
    \item Analise o erro da interpolação.
    \item Compare graficamente os resultados.
\end{itemize}

\subsection*{Interpolação de Newton}

\item Sejam os vetores:
\[
x = [1, 2, 3], \quad y = [1, 8, 27]
\]

\begin{itemize}
    \item Construa a tabela de diferenças divididas.
    \item Determine o polinômio interpolador de Newton.
\end{itemize}

\item Sejam os vetores:
\[
x = [1, 2, 4], \quad y = [2, 3, 7]
\]

\begin{itemize}
    \item Construa o polinômio interpolador de Newton.
    \item Estime $f(1.5)$.
\end{itemize}

\subsection*{Spline Linear}

\item Sejam os vetores:
\[
x = [0, 1, 2], \quad y = [1, 3, 2]
\]

\begin{itemize}
    \item Construa o spline linear.
    \item Analise o erro associado.
    \item Apresente o resultado graficamente.
\end{itemize}

\item Sejam os vetores:
\[
x = [1, 2], \quad y = [2, 4]
\]

\begin{itemize}
    \item Construa o spline linear.
    \item Estime $f(1.5)$.
    \item Analise o erro e represente graficamente.
\end{itemize}

\subsection*{Spline Quadrática}

\item Sejam os vetores:
\[
x = [0, 1, 2], \quad y = [1, 2, 0]
\]

\begin{itemize}
    \item Construa o spline quadrático com continuidade da primeira derivada.
    \item Analise o erro.
    \item Apresente os resultados graficamente.
\end{itemize}

\item Sejam os vetores:
\[
x = [1, 2, 3], \quad y = [1, 3, 2]
\]

\begin{itemize}
    \item Determine os coeficientes do spline quadrático.
    \item Analise o erro.
    \item Apresente os resultados graficamente.
\end{itemize}

\subsection*{Spline Cúbica}

\item Sejam os vetores:
\[
x = [0, 1, 2, 3], \quad y = [0, 1, 0, 1]
\]

\begin{itemize}
    \item Construa o spline cúbico natural.
    \item Analise o erro.
    \item Apresente os resultados graficamente.
\end{itemize}

\item Sejam os vetores:
\[
x = [1, 2, 3, 4], \quad y = [2, 3, 5, 4]
\]

\begin{itemize}
    \item Construa o spline cúbico com condições naturais.
    \item Analise o erro.
    \item Apresente os resultados graficamente.
\end{itemize}

\item Sejam os vetores:
\[
x = [-1, 0, 1, 2, 3], \quad y = [1, 0, 1, 8, 27]
\]

\begin{itemize}
    \item Utilizando programação orientada a objetos (POO), desenvolva um \textit{script} que implemente:
    \begin{itemize}
        \item Regressão polinomial
        \item Interpolação de Lagrange
        \item Interpolação de Newton
        \item Splines (linear, quadrática e cúbica)
    \end{itemize}
    \item Compare os resultados entre os métodos.
    \item Apresente análise gráfica e quantitativa dos erros.
\end{itemize}

\item Apresente um comparativo gráfico entre os resultados do questão anterior.

\end{enumerate}
\section{Soluções de Equações Lineares} \label{sec:SolSistLineares}

\begin{enumerate}[label=\thesection.\arabic*]
\item Considere:
\[
A =
\begin{bmatrix}
2 & -1 & 1 \\
3 & 3 & 9 \\
3 & 3 & 5
\end{bmatrix},
\quad
b =
\begin{bmatrix}
2 \\ -1 \\ 4
\end{bmatrix}
\]

\begin{itemize}
    \item Implemente um \textit{script} com o método da eliminação de Gauss sem pivoteamento.
    \item Resolva o sistema $Ax = b$.
    \item Apresente a matriz triangular superior obtida.
    \item Valide o resultado calculando o resíduo $\|Ax - b\|$.
\end{itemize}

\item Considere:
\[
A =
\begin{bmatrix}
1 & 2 & 3 \\
2 & 5 & 2 \\
3 & 1 & 5
\end{bmatrix},
\quad
b =
\begin{bmatrix}
14 \\ 18 \\ 20
\end{bmatrix}
\]

\begin{itemize}
    \item Implemente a fatoração LU (sem pivoteamento).
    \item Resolva o sistema utilizando a decomposição $A = LU$.
    \item Resolva também utilizando \texttt{numpy.linalg.solve}.
    \item Compare as soluções obtidas e o erro numérico.
\end{itemize}

\item Considere o sistema linear:
\[
A =
\begin{bmatrix}
10 & -1 & 2 & 0 \\
-1 & 11 & -1 & 3 \\
2 & -1 & 10 & -1 \\
0 & 3 & -1 & 8
\end{bmatrix},
\quad
b =
\begin{bmatrix}
6 \\ 25 \\ -11 \\ 15
\end{bmatrix}
\]

\begin{itemize}
    \item Implemente o método de Jacobi num \textit{script}.
    \item Utilize um critério de parada baseado em $\|x^{(k+1)} - x^{(k)}\|$.
    \item Determine o número de iterações necessário para convergência.
    \item Compare com a solução obtida via \texttt{numpy.linalg.solve} do python ou similar.
\end{itemize}

\item Considere:
\[
A =
\begin{bmatrix}
4 & 1 \\
2 & 3
\end{bmatrix},
\quad
b =
\begin{bmatrix}
1 \\ 2
\end{bmatrix}
\]

\begin{itemize}
    \item Implemente os métodos de Jacobi e Gauss-Seidel.
    \item Compare o número de iterações até a convergência.
    \item Analise a taxa de convergência dos métodos.
    \item Valide os resultados com a solução exata.
\end{itemize}

\item Considere:
\[
A =
\begin{bmatrix}
0 & 2 & 3 \\
4 & 5 & 6 \\
7 & 8 & 9
\end{bmatrix},
\quad
b =
\begin{bmatrix}
6 \\ 15 \\ 24
\end{bmatrix}
\]

\begin{itemize}
    \item Implemente a eliminação de Gauss com pivoteamento parcial.
    \item Resolva o sistema e apresente a solução.
    \item Compare com a solução obtida sem pivoteamento.
    \item Calcule o resíduo em ambos os casos.
    \item Discuta os efeitos de estabilidade numérica observados.
\end{itemize}

\end{enumerate}

\section{Soluções de equações não lineares} \label{sec:SolSistNaoLineares}

\begin{enumerate}[label = \thesection.\arabic*]
	
\item Considere a função:
\[
f(x) = x^3 - x - 2
\]

\begin{itemize}
    \item Implemente o método da bisseção.
    \item Determine uma raiz no intervalo $[1,2]$ com tolerância $10^{-5}$.
    \item Apresente o número de iterações.
    \item Verifique o erro final.
\end{itemize}

\item Considere:
\[
f(x) = e^{-x} - x
\]

\begin{itemize}
    \item Implemente o método da falsa posição.
    \item Determine a raiz no intervalo $[0,1]$.
    \item Compare o número de iterações com o método da bisseção.
    \item Analise a eficiência dos métodos.
\end{itemize}

\item Considere:
\[
f(x) = x^3 - 2x - 5
\]

\begin{itemize}
    \item Implemente o método de Newton-Raphson.
    \item Utilize $x_0 = 2$ como aproximação inicial.
    \item Estabeleça critério de parada baseado em erro relativo.
    \item Analise a convergência do método.
\end{itemize}

\item Considere:
\[
f(x) = \cos(x) - x
\]

\begin{itemize}
    \item Implemente o método do ponto fixo.
    \item Escolha uma função de iteração $g(x)$ adequada.
    \item Verifique a condição de convergência $|g'(x)| < 1$.
    \item Compare com o método de Newton.
\end{itemize}

\item Considere:
\[
f(x) = x^3 - 7x + 6
\]

\begin{itemize}
    \item Implemente o método da secante.
    \item Utilize $x_0 = 0$ e $x_1 = 1$.
    \item Determine uma raiz com tolerância $10^{-6}$.
    \item Compare com Newton-Raphson em termos de iterações.
\end{itemize}

\item Considere:
\[
f(x) = \ln(x) + x^2 - 3
\]

\begin{itemize}
    \item Implemente os métodos da bisseção e de Newton.
    \item Compare as soluções obtidas.
    \item Analise a velocidade de convergência.
    \item Apresente os resultados graficamente.
\end{itemize}

\item Considere o sistema não linear:
\[
\begin{cases}
x^2 + y^2 = 4 \\
x - y = 1
\end{cases}
\]

\begin{itemize}
    \item Implemente o método de Newton para sistemas não lineares.
    \item Construa a matriz Jacobiana.
    \item Utilize uma estimativa inicial adequada.
    \item Resolva o sistema e valide a solução.
\end{itemize}

\item Considere:
\[
f(x) = x - \tan(x)
\]

\begin{itemize}
    \item Implemente os métodos da bisseção, secante e Newton.
    \item Compare o número de iterações para convergência.
    \item Analise possíveis dificuldades numéricas (descontinuidades).
    \item Discuta a robustez de cada método.
\end{itemize}

% ==========================================================================
\end{enumerate}